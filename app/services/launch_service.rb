class LaunchService
  ...
  def authenticate_launch
    @launch = Launch.create(payload: payload)
    find_credential
  ...
  def find_credential
    raise "No LTI key" unless payload[:oauth_consumer_key].present?

    unless @credential = Credential.find_by(
      consumer_key: payload[:oauth_consumer_key],
    )
  ...
  def find_or_sign_up_user
    @user = User.find_or_create_by(id_from_tc: payload[:user_id]) do |user|
